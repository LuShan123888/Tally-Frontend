<template>
  <v-container :class="{ 'ml-10': !isMobile }" :style="{ width: isMobile ? '100%' : '50%' }" class="py-0 px-4" fluid>
    ...
    <v-btn color="primary" fab fixed right style="bottom: 68px" @click="loadSaveBillPage">
      <v-icon>mdi-plus</v-icon>
    </v-btn>
    <v-row class="flex" no-gutters>
      <div class="pr-1" style="width: 115px">
        <v-menu max-width="290px" min-width="auto" offset-y transition="scale-transition">
          <template v-slot:activator="{ on, attrs }">
            <v-text-field v-model="query.dateQueryString" dense flat label="请选择月份" prepend-inner-icon="mdi-calendar"
                          readonly solo v-bind="attrs" v-on="on"></v-text-field>
          </template>
          <v-date-picker v-model="query.dateQueryString" color="primary" locale="zh-cn" no-title scrollable
                         type="month"></v-date-picker>
        </v-menu>
      </div>
      <div class="pl-1 flex-grow-1">
        <v-text-field v-model="query.description" clearable dense flat label="请输入关键字" prepend-inner-icon="mdi-magnify"
                      solo></v-text-field>
      </div>
    </v-row>
    <v-card class="pa-0 rounded-lg" flat fluid>
      ...
      <v-card-subtitle class="pa-3 pt-1 ma-0">
        <v-row no-gutters>
          <v-col cols="6"><span class="font-weight-medium">总收入</span><span class="ml-3"
                                                                           v-text="'¥' + numFormat(billSummaryData.income)"/>
          </v-col>
          <v-col cols="6"><span class="font-weight-medium">结余</span><span class="ml-3"
    ...
    <v-form ref="saveOrUpdateBudgetForm">
      <v-dialog v-model="budgetBar.dialog.isShow" max-width="600px" persistent>
        <v-card class="rounded-lg">
          <v-card-title><span v-text="budgetBar.dialog.title"/></v-card-title>
          <v-card-text class="pb-0">
            <v-container class="pa-0">
              <v-row no-gutters>
                <v-col cols="12">
                  <v-text-field v-model="budgetBar.dialog.budget.amount"
                                :rules="[(value) => !!value || '请输入预算金额',rules.isPositive]" clearable label="预算金额"
          ...
          <v-card-actions>
            <v-spacer/>
            <v-btn class="rounded-lg" depressed @click="budgetBar.dialog.isShow = false" v-text="'取消'"/>
            <v-btn :disabled="budgetBar.dialog.loading" :loading="budgetBar.dialog.loading" class="rounded-lg"
                   color="primary" depressed @click="saveOrUpdateBudget" v-text="'保存'"/>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-form>
    <div v-if="loading">
      <v-skeleton-loader v-for="item in 6" :key="'skeleton-loader' + item" class="rounded-lg mt-4"
                         type="list-item-avatar-two-line"/>
    </div>
    <div v-if="!loading">
      <v-container v-for="(date, i) in billList" :key="i" class="pa-0" fluid>
        <v-row align="center" class="px-0 py-3 font-weight-medium d-flex justify-space-between" no-gutters
               style="position: relative"><span class="text-subtitle-2 text--darken-1 grey--text"
                                                v-text="date.groupName"/><span
            class="text-subtitle-2 text--darken-1 grey--text"><span>结余</span><span class="ml-2"
                                                                                   v-text="'¥' + numFormat(date.amount)"/></span>
        </v-row>
        <v-card class="py-0 rounded-lg" flat fluid>
          <v-container v-for="(item, i) in date.list" :key="i" v-ripple class="pa-0 px-3" fluid>
            <v-row align="center" no-gutters style="height: 60px; cursor: pointer" @click="loadUpdateBillPage(item)">
              <v-col cols="1">
                <v-btn
              ...
              <v-col class="d-flex justify-end" cols="3">
                <div class="text-subtitle-1"><span
                    v-text=" (item.flow === 'OUT' ? '¥-' : '¥') + numFormat(item.amount)"/></div>
              </v-col>
    ...
    <v-dialog ref="dialog" v-model="billPage.isShow" eager fullscreen hide-overlay
              transition="dialog-bottom-transition">
      <el-upload :action="upload.path" :before-upload="beforeAvatarUpload" :headers="upload.header"
                 :on-error="handleAvatarError" :on-success="handleAvatarSuccess" :show-file-list="false"
                 :style="{ height: 0 + 'px', width: 0 + 'px', }" name="image">
        <v-btn :loading="upload.loading" color="primary" fab fixed right style="bottom: 68px">
              ...
              <v-form ref="outBillSaveOrUpdateForm">
                <v-card class="pa-4 rounded-lg" flat>
                  <v-row no-gutters>
                    <v-col class="my-3" cols="6">
                      <v-select v-model="billPage.bill.billTypeVO.parentId" :items="billPage.outBillTypeTree"
                                :rules="[(value) => !!value || '请选择主账单类别']" chips class="pr-1" dense
                                item-text="billTypeName" item-value="id" label="主账单类别" no-data-text="无对应选项"
                                prepend-inner-icon="mdi-format-list-bulleted-type"
                                @change="billPage.bill.billTypeVO.id = null"></v-select>
                    </v-col>
                    <v-col class="mb-3" cols="6">
                      ...
                      <v-row justify="center" no-gutters>
                        <v-btn class="rounded-lg" color="error" depressed fab x-small>
                          <v-icon v-if="billPage.bill.billTypeVO.icon" v-text="'mdi-' + billPage.bill.billTypeVO.icon"/>
                          <v-icon v-else>mdi-help</v-icon>
                        </v-btn>
                      </v-row>
                    </v-col>
                    <v-col class="pr-1" cols="6">
                      <v-select v-model="billPage.bill.billTypeVO.id" :items="billPage.billTypeChildren" chips
                                class="pl-1" clearable dense item-text="billTypeName" item-value="id" label="子账单类别"
                                no-data-text="无对应选项" prepend-inner-icon="mdi-format-list-bulleted-type"/>
                    </v-col>
                    <v-col class="pl-1" cols="6">
                      <v-select v-model="billPage.bill.outAccountId" :items="billPage.accountList"
                                :rules="[(value) => !!value || '请选择转出账户']" chips class="pr-1" dense
                                item-text="accountName" item-value="id" label="转出账户" no-data-text="无对应选项"
                                prepend-inner-icon="mdi-wallet"/>
                    </v-col>
                    <v-col class="pr-1" cols="6">
                      <v-menu max-width="290px" min-width="auto" offset-y transition="scale-transition">
                        <template v-slot:activator="{ on, attrs }">
                          <v-text-field v-model="billPage.bill.billDate" label="请选择账单日期"
                                        prepend-inner-icon="mdi-calendar" readonly v-bind="attrs"
                                        v-on="on"></v-text-field>
                        </template>
                        <v-date-picker v-model="billPage.bill.billDate" color="primary" locale="zh-cn" no-title
                                       scrollable type="date"></v-date-picker>
                      </v-menu>
                    </v-col>
                    <v-col class="pl-1" cols="6">
                      <v-text-field v-model="billPage.bill.amount"
                                    :rules="[ (value) => !!value || '请输入金额', rules.isPositive, ]" clearable label="账单金额"
                                    prefix="¥" prepend-inner-icon="mdi-currency-usd"></v-text-field>
                    </v-col>
                    <v-col cols="12">
                      <v-text-field v-model="billPage.bill.description" clearable label="账单描述" messages=" "
                        ...
                        <template v-slot:message="{  }">
                          <div class="overflow-auto" style="white-space: nowrap">
                            <v-chip v-for="item in billPage.recentBillDescriptionList" :key="item" class="mr-1" small
                                    @click="billPage.bill.description = item">{{ item }}
                            </v-chip>
                          </div>
                        </template>
                      </v-text-field>
                ...
                <v-row class="mt-4" no-gutters>
                  <v-col v-if="billPage.type === 'update'" class="pr-2" cols="6">
                    <v-btn :disabled="billPage.buttons.delete.loading" :loading="billPage.buttons.delete.loading" block
                           class="rounded-lg" color="error" depressed large @click="deleteBill(billPage.bill.id)">
                      <v-icon class="mr-3">mdi-delete</v-icon>
                      <span>删除</span></v-btn>
                  </v-col>
                  <v-col :class="{ 'pl-2': billPage.type === 'update' }" :cols="billPage.type === 'update' ? 6 : 12">
                    <v-btn :disabled="billPage.buttons.saveOrUpdate.loading"
                           :loading="billPage.buttons.saveOrUpdate.loading" block class="rounded-lg" color="primary"
                           depressed large @click="saveOrUpdateBill('OUT')">
            ...
            <v-container :style="{ width: isMobile ? '100%' : '50%' }" class="mx-auto px-4 py-0" fluid no-gutters>
              <v-form ref="inBillSaveOrUpdateForm">
                <v-card class="pa-4 rounded-lg" flat>
                  <v-row no-gutters>
                    <v-col class="my-3" cols="6">
                      <v-select v-model="billPage.bill.billTypeVO.parentId" :items="billPage.inBillTypeTree"
                                :rules="[(value) => !!value || '请选择主账单类别']" chips class="pr-1" dense
                                item-text="billTypeName" item-value="id" label="主账单类别" no-data-text="无对应选项"
                                prepend-inner-icon="mdi-format-list-bulleted-type"
                    ...
                    <v-col class="pl-1" cols="6">
                      <v-select v-model="billPage.bill.inAccountId" :items="billPage.accountList"
                                :rules="[(value) => !!value || '请选择转入账户']" chips class="pr-1" dense
                                item-text="accountName" item-value="id" label="转入账户" no-data-text="无对应选项"
                                prepend-inner-icon="mdi-wallet"/>
                    </v-col>
                    <v-col class="pr-1" cols="6">
                      <v-menu max-width="290px" min-width="auto" offset-y transition="scale-transition">
                ...
                <v-row class="mt-4" no-gutters>
                  ...
                  <v-col :class="{ 'pl-2': billPage.type === 'update' }" :cols="billPage.type === 'update' ? 6 : 12">
                    <v-btn :disabled="billPage.buttons.saveOrUpdate.loading"
                           :loading="billPage.buttons.saveOrUpdate.loading" block class="rounded-lg" color="primary"
                           depressed large @click="saveOrUpdateBill('IN')">
                      <v-icon class="mr-3">mdi-content-save</v-icon>
                      <span>保存</span></v-btn>
                  </v-col>
                </v-row>
              ...
              <v-form ref="transferBillSaveOrUpdateForm">
                  ...
                  <v-row no-gutters>
                    ...
                    <v-col class="mt-3" cols="6">
                      <v-select v-model="billPage.bill.inAccountId" :items="billPage.accountList"
                                ...
                                chips class="pr-1" dense item-text="accountName" item-value="id" label="转入账户"
                                no-data-text="无对应选项" prepend-inner-icon="mdi-wallet"/>
                    </v-col>
                    <v-col class="pr-1" cols="6">
                      <v-menu max-width="290px" min-width="auto" offset-y transition="scale-transition">
                        <template v-slot:activator="{ on, attrs }">
                          <v-text-field v-model="billPage.bill.billDate" label="请选择账单日期" prepend-icon="mdi-calendar"
                ...
                <v-row class="mt-4" no-gutters>
                  ...
                  <v-col :class="{ 'pl-2': billPage.type === 'update' }" :cols="billPage.type === 'update' ? 6 : 12">
                    <v-btn :disabled="billPage.buttons.saveOrUpdate.loading"
                           :loading="billPage.buttons.saveOrUpdate.loading" block class="rounded-lg" color="primary"
                           depressed large @click="saveOrUpdateBill('TRANSFER')">
                      <v-icon class="mr-3">mdi-content-save</v-icon>
                      <span>保存</span></v-btn>
                  </v-col>
                </v-row>
